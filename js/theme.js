jQuery(document).ready(function() {
	...
	jQuery('.theme-search-box').each(function(index, element) {
		...
		function collapse() {
			...
			searchBox.stop(true, true).animate({width: '75'}, ANIMATE_SPEED);
		}
...
element.hover(function() {
	...
	var ftHeight = jQuery('#theme-footer-tabs ul').height() + 8;
	...
	jQuery('.portfolio-container .item a').each(function(index, element) {
		...
		element.click(function(e){
			...
			jQuery('.portfolio_block ' + _target ).load( url + " .theme-article-content", function(response, status, xhr) {
				...
				jQuery('#types li').animate({'margin-top': '0.47%'});
	...
	jQuery('.navi .close').click(function () {
		...
		jQuery('#types li').animate({'margin-top': '0'});
	...
	jQuery(".slidepanel-arrow").click(function(){
		if(jQuery(slidepanel).css('display') == 'none'){
			...
			jQuery(slidepanel).find('.slidepanel-arrow').show();
		}else{
...
jQuery(window).load(function() {
	...
	jQuery('.theme-search-box').animate({height: searchboxHeight + 7 }, ANIMATE_SPEED);
	jQuery('.theme-search-box input[type="text"]').animate({height: searchboxHeight + 7 }, ANIMATE_SPEED);
	jQuery('.theme-search-icon').animate({height: searchboxHeight + 7 }, ANIMATE_SPEED);
...
_rys("document").ready(function(){
	...
	if(!isiPad || !isiPhone || !isiPod){
		_rys(window).scroll(function () {
	    	var height = jQuery('#theme-logo').height() + jQuery('#theme-navigation-row').height();
	        if (_rys(this).scrollTop() > height) {
	            ...
	            setTimeout(collapse, DELAY_COLLAPSE);
	            _rys('#theme-navigation').stop(true, true).animate({'margin-left':  (_rys('#theme-logo-menu img').width() + 20) + 'px'}, ANIMATE_SPEED);
	            setTimeout(collapse, DELAY_COLLAPSE);
	            _rys('#theme-logo-menu').stop(true, true).animate({'margin-left':'1.5%',opacity: 1}, ANIMATE_SPEED);
	           setTimeout(collapse, DELAY_COLLAPSE);
	            _rys('#theme-logo-menu img').stop(true, true).delay(600).animate({opacity: 1}, ANIMATE_SPEED);
	        } else {
	            _rys('#theme-navigation-row').removeClass("f-nav");
	            _rys('#theme-navigation-row').css('margin-top', 0 );
	            setTimeout(collapse, DELAY_COLLAPSE);
	            _rys('#theme-logo-menu img').stop(true, true).animate({opacity: 0}, ANIMATE_SPEED).delay(600);
	            _rys('#theme-logo-menu').stop(true, true).animate({opacity: 0}, ANIMATE_SPEED);
	            _rys('#theme-navigation').stop(true, true).animate({'margin-left': '0px'}, ANIMATE_SPEED);
